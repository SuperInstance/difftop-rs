...
impl Atlas {
    ...
    pub fn add_transition(
        ...
        i: usize,
        j: usize,
        jacobian: impl Fn(&DVector<f64>) -> DMatrix<f64> + 'static,
    ) {
...
pub fn circle_manifold() -> SmoothManifold {
    ...
    m.atlas
        .add_transition(c0, c1, |_: &DVector<f64>| DMatrix::identity(1, 1));
    m.atlas
        .add_transition(c1, c0, |_: &DVector<f64>| DMatrix::identity(1, 1));
    m
}
...
pub fn torus_manifold() -> SmoothManifold {
    ...
    m.atlas
        .add_transition(c0, c1, |_: &DVector<f64>| DMatrix::identity(2, 2));
    m.atlas
        .add_transition(c1, c0, |_: &DVector<f64>| DMatrix::identity(2, 2));
    m
}
...
pub fn n_sphere_manifold(n: usize) -> SmoothManifold {
    ...
    match n {
        1 => {
            m.atlas.add_transition(c0, c1, |_p: &DVector<f64>| {
                DMatrix::from_row_slice(1, 1, &[1.0])
            });
            m.atlas.add_transition(c1, c0, |_p: &DVector<f64>| {
                DMatrix::from_row_slice(1, 1, &[1.0])
            });
        }
        2 => {
        ...
        _ => {
            ...
            let dim = n;
            m.atlas
                .add_transition(c0, c1, move |_: &DVector<f64>| DMatrix::identity(dim, dim));
            let dim = n;
            m.atlas
                .add_transition(c1, c0, move |_: &DVector<f64>| DMatrix::identity(dim, dim));
        }
    }
    m
}
